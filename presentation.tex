...
\note[itemize]{
\item Time Series Forecasting is one of the most necessary activities
\item Forecasting is spread over many knowledge fields, engineering, economy, etc.
\item Despite its importance, not so many people take in consideration the uncertainty inherent to the forecasting 
\item Intrinsic is proper to the stochastic processes, its non-determinism, etc.
\item Extrinsic uncertainty originates outside the process, is caused by instrumentation, measurement, data storage, etc
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Introduction - Uncertainty}
\linespread{2}
Why it is important to know the uncertainty?
\begin{itemize}
...
\note[itemize]{
\item We imposed some constraints in the FTS components to close the research scope and make it more robust
...
\note[itemize]{
\item This diagram shows the traning ....  
}
