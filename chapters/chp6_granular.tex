...
\index{$\mlvar$}\index{Linguistic variable}
...
\item[Stage 2] \textit{Fuzzyfication}: \index{Fuzzyfication}
...
\item[Stage 3] \textit{Rule Induction}: \index{Rule induction}
\begin{enumerate}
...
\begin{enumerate} \index{Fuzzyfication}
...
\end{table}
\index{$\kappa$}\index{Hyperparameter}
...
\begin{enumerate}
\item[Stage 1] \textit{Partitioning}:\index{Partitioning}
\begin{enumerate}
\item \textit{Defining $U_{\vari}$}: The Universe of Discourse $U_{\vari}$ defines the sample space, i.e., the known bounds of the variable $\vari$, such that $U_{\vari} = [\min(Y^{\vari})-D_1, \max(Y^{\vari})+D_2]$, where $D_1 = \min(Y^{\vari})\times 0.2$ and $D_2 = \max(Y^{\vari})\times 0.2$ are used to extrapolate the known bounds as a security margin, $\forall \vari \in \var$.
...
\index{Multivariate forecasting}
...
\index{Many steps ahead forecasting}
